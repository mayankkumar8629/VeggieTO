      ...
      {dialogOpen && (
        <div className="fixed inset-0 z-50 backdrop-blur-md bg-black/30 flex items-center justify-center px-4">
          <div className="relative bg-white w-full max-w-3xl rounded-xl shadow-xl flex flex-col md:flex-row overflow-y-auto max-h-[90vh]">
            <button onClick={() => setDialogOpen(false)} className="absolute top-4 right-4 text-gray-500 hover:text-gray-800 text-2xl">&times;</button>

            <div className="hidden md:flex flex-col items-center justify-end flex-initial">
              <img src={login} alt="Dialog" className="w-full h-96 object-contain" />
            </div>
            ...
            <div className="flex-1 ml-0 md:ml-4 p-8 flex items-center justify-center">
              {authTab === 'signup' ? (
                <form className="flex flex-col items-center w-full" onSubmit={handleSignup}>
                  <div className="flex justify-between mb-6">
                    {['customer', 'vendor', 'admin'].map((role) => (
                      <button key={role} type="button"
                        className={`px-3 py-2 mx-2 rounded-full border ${userRole === role ? 'bg-[#4CAF50] text-white border-[#4CAF50]' : 'bg-white text-gray-700 border-gray-300'}`}
                  ...
                  <button type="submit" className="bg-[#4CAF50] text-white rounded-full px-4 py-2 mb-4 w-64">Create Account</button>
                  <p className="text-gray-500 mb-4">Already have an account? <span className="text-[#4CAF50] cursor-pointer" onClick={() => switchTab('login')}>Login</span></p>
                </form>
              ) : (
                <form className="flex flex-col items-center w-full" onSubmit={handleLogin}>
                  <div className="flex justify-center mb-6">
                    {['customer', 'vendor', 'admin'].map((role) => (
                      <button key={role} type="button"
                        className={`px-4 py-2 mx-2 rounded-full border ${userRole === role ? 'bg-[#4CAF50] text-white border-[#4CAF50]' : 'bg-white text-gray-700 border-gray-300'}`}
                        onClick={() => setUserRole(role)}>
                        {role.charAt(0).toUpperCase() + role.slice(1)}
                      </button>
                    ))}
                  ...
                  <button type="submit" className="bg-[#4CAF50] text-white rounded-full px-4 py-2 mb-4 w-64">Login</button>
                  <p className="text-gray-500 mb-4">Don't have an account? <span className="text-[#4CAF50] cursor-pointer" onClick={() => switchTab('signup')}>Signup</span></p>
                </form>
              )}
            </div>
          </div>
